    ...
    class UserInteraction:
        ...
        def call_list(self, *args):
            ...
            result_list_function = list_choices.get(
                self.input_command, UI.call_check_project_context)
        ...
        def call_extend(self):
            ...
            valid_flag, serial_number, new_date = todo.check_valid_extend(
                self.input_command)
            if(valid_flag):
 ...
 and date, run todo help ext for more\n')
        ...
        def call_check_project_context(self):
            ...
            if(self.input_command.split(' ')[0] == 'due'):  # check if it's for a day/date
                search_str = r'(today|tomorrow|[\d]{1,2}\s[jan,feb,mar\,apr,may,jun,jul,aug,sep,oct,nov,dec]{3})'
                if(len(re.findall(search_str, self.input_command)) > 0):
                    ...
                    print(
                        'not a valid choice, try with a different due [value]')
            else:  # check for project or context
                value_type, check_result = check_project_context(
                    self.input_command)
                if(check_result == 'False'):
                    ...
                    if(value_type == 'project' and todo.check_valid_project_name(check_result)):
                        ...
                        print(
                            'No such project/context/option exist, run todo help list for more')
